...
// simple helper method to randomly choose from any given array
function randomlyChooseFromArray (array) {
    return array[Math.floor(Math.random() * array.length)]
};

// simple helper - random number generator
function randomlyChooseNumber (number) {
    return Math.floor(Math.random() * number)
};

// the 'AI' sitting behin the fight sequence generation
function generateRandomFightSequence (user1, user2) {

    let coinFlip = Math.random()
    // player 1 is the person who starts the fight
    const player1 = coinFlip < 0.5 ? user1 : user2;
    const player2 = player1 === user1 ? user2 : user1;

    function takedownAction (attacker, defender) {

        attackSequence.push([fight.attack.wrestling.submissionAttempt, attacker]);
        let submissionAttemptResponse = randomlyChooseNumber(3)

        switch (submissionAttemptResponse) {
            // defender submission defence
            case 0: attackSequence.push([fight.defence.wrestling.submissionDefence, defender]); break;
            // defender scrambles
            case 1: attackSequence.push([fight.counter.wrestling.submissionScramble, defender]);break;
            // attacker ground and pound (success)
            case 2: attackSequence.push([fight.success.wrestling.groundAndPound, attacker]);break;
        };
    }

    function fightAction (attacker, defender) {

        let fightActionResponse = randomlyChooseNumber(3)

        switch (fightActionResponse) {
            case 0: attackSequence.push([fight.attack.strike.strikeAttackKick, attacker]); break;
            case 1: attackSequence.push([fight.attack.strike.strikeAttackPunch, attacker]);break;
            case 2: attackSequence.push([fight.attack.wrestling.takedownAttempt, attacker]);break;
        };

        switch(attackSequence[attackSequence.length - 1][0]) {

            case fight.attack.strike.strikeAttackKick:

                strikeAttackKickResponse = randomlyChooseNumber(7)
    
                switch (strikeAttackKickResponse) {
                    case 0: case 1: attackSequence.push([fight.defence.strike.strikeDefenceKick, defender]); break;
                    case 2: case 3: attackSequence.push([fight.defence.strike.strikeDefenceKick, defender]); break;
                    case 4: case 5: attackSequence.push([fight.counter.strike.counterKick, defender]); break;
                    case 6: attackSequence.push([fight.success.strike.strikeSuccessKick, attacker]); break;
                }; break;
                
            case fight.attack.strike.strikeAttackPunch:

                strikeAttackPunchResponse = randomlyChooseNumber(7)
    
                switch (strikeAttackPunchResponse) {
                    case 0: case 1: attackSequence.push([fight.defence.strike.strikeDefencePunch, defender]); break;
                    case 2: case 3: attackSequence.push([fight.defence.strike.strikeDefencePunch, defender]); break;
                    case 4: case 5: attackSequence.push([fight.counter.strike.counterPunch, defender]); break;
                    case 6: attackSequence.push([fight.success.strike.strikeSuccessPunch, attacker]); break;
                }; break;

            case fight.attack.wrestling.takedownAttempt:

                takedownAttemptResponse = randomlyChooseNumber(3)

                switch (takedownAttemptResponse) {
                    case 0: case 1: attackSequence.push([fight.defence.wrestling.takedownDefence, defender]); break;
                    case 2: attackSequence.push([fight.success.wrestling.takedownSuccess, attacker]);
                    takedownAction(attacker, defender); break;
                }; break;
    ...
    // fightAction(player1, player2)
    fightAction(player1, player2)
    fightAction(player2, player1)
    fightAction(player1, player2)
    fightAction(player2, player1)
...
// this will be where I use jQuery to implement the fight on screen
function describeFight () {

    for (let index = 0; index < attackSequence.length; index++) {
        
        const fighterToDescribe = attackSequence[index][1];
        const actionToDescribe = attackSequence[index][0];
        
        console.log(fighterToDescribe, randomlyChooseFromArray(actionToDescribe))
    }

};

generateRandomFightSequence('Dan', 'Luke')